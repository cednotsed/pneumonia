...
cwd = Path.cwd()
print(cwd)
datasets = cwd / '../results/amr_out/amr_matrices'
results = cwd / '../results/ML_out/amr_out'
...
## Data Preprocessing
### Load data
raw_df = pd.read_csv(datasets / f'resfinder.amr.decontam.csv')
...
# Export results
print(results / f'shap_out/{g1}.{g2}.shap.csv')
merged.to_csv(results / f'shap_out/{g1}.{g2}.shap.csv', index=False)
final_results.to_csv(results / f'results_out/{g1}.{g2}.results.csv', index=False)
...
plt.savefig(results / f'beeswarm_out/{g1}.{g2}.pdf', dpi=600, format='pdf')
