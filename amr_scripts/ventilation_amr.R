...
df <- fread("results/amr_out/resfinder_out/resfinder.parsed.decontam.csv") %>%
  ...
  left_join(meta %>%
              select(run_id, hap_vap2, ventilation))
...
plot_df %>%
  mutate(class = factor(class, abx_counts$class)) %>%
...
ggsave("results/amr_out/amr_profiles.ventilation.pdf", dpi = 600, width = 5, height = 3)

# Permutation test
observed <- plot_df %>%
  pivot_wider(id_cols = class, names_from = ventilation, values_from = perc) %>%
...
perm_morsels <- foreach(i = seq(n_iters)) %do% {
  perm_labels <- meta %>% 
    ...
    select(run_id, ventilation)
  
  perm_labels <- perm_labels %>%
    mutate(ventilation = sample(perm_labels$ventilation, length(perm_labels$ventilation), replace = F))
