...
file_dir <- "results/amr_out/resfinder_out.assemblies.c0.8/all_reports/"
...
parsed <- res_df %>%
  ...
  mutate(genome_name = gsub(".pheno_table.txt", "", run_name)) %>%
  mutate(run_id = gsub("INHALE_FRESH_|.no_human|Library|_raw", "", genome_name)) %>%
...
parsed %>%
  fwrite("results/amr_out/resfinder_out/resfinder.assemblies.c0.8.parsed.csv")
