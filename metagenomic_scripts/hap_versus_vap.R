...
# Permutation test
observed <- plot_df %>%
  pivot_wider(id_cols = bugs, names_from = hap_vap_cap, values_from = perc) %>%
...
perm_morsels <- foreach(i = seq(n_iters)) %do% {
  perm_labels <- meta_filt %>%
    mutate(hap_vap_cap = sample(meta_filt$hap_vap_cap, 
                                length(meta_filt$hap_vap_cap), 
                                replace = F)) %>%
    dplyr::select(run_id, hap_vap_cap)
...
pval_df %>%
  fwrite("results/metagenomic_out/hap_vap_bug_permutation.PCR.csv")
...
plot_df %>%
  ...
  ggplot(aes(x = hap_vap_cap, y = bugs, fill = perc)) +
...
ggsave("results/metagenomic_out/hap_vap_pathogens.PCR.pdf", dpi = 600,
       height = 3, width = 5)
